...
感谢你对 Ottie 的关注！本项目是一个社区驱动的开源项目，目标是构建 轻量灵活,人人可用 的个人AI助手。我们欢迎一切形式的贡献：Bug 修复、新功能、文档、翻译和测试。

Ottie 本身在很大程度上是借助 AI 辅助开发的——我们拥抱这种方式，并围绕它构建了贡献流程。
...
- **测试与验证** — 在新硬件、新渠道或新 LLM 提供商上运行 Ottie 并反馈结果。
...
2. 将你的 Fork **克隆**到本地：
   ```bash
   git clone https://github.com/<你的用户名>/ottie.git
   cd ottie
   ```
3. 添加上游远程仓库：
   ```bash
...
Ottie 在很大程度上借助 AI 辅助开发，我们完全拥抱这种开发方式。但贡献者必须清楚地了解自己在使用 AI 工具时所承担的责任。
...
Ottie 的架构在人工监督下，经由 AI 辅助完成了大量设计和实现工作。如果你发现某处看起来奇怪或过度设计，这可能是该过程留下的痕迹——欢迎提 Issue 讨论。
