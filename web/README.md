...
This directory contains the standalone web service for `ottie`.
It provides a complete unified web interface, acting as a dashboard, configuration center, and interactive console (channel client) for the core `ottie` engine.
...
*   **`backend/`**: The Go-based web server. It provides RESTful APIs, manages WebSocket connections for chat, and handles the lifecycle of the `ottie` process. It eventually embeds the compiled frontend assets into a single executable.
...
The output binary is `backend/ottie-web`.
