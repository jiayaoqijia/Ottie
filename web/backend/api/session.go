...
// ottieSessionPrefix is the key prefix used by the gateway's routing for Ottie
// channel sessions. The full key format is:
//
//	agent:main:ottie:direct:ottie:<session-uuid>
//
// The sanitized filename replaces ':' with '_', so on disk it becomes:
//
//	agent_main_ottie_direct_ottie_<session-uuid>.json
const (
	ottieSessionPrefix          = "agent:main:ottie:direct:ottie:"
	sanitizedOttieSessionPrefix = "agent_main_ottie_direct_ottie_"
	maxSessionJSONLLineSize     = 10 * 1024 * 1024 // 10 MB
	maxSessionTitleRunes        = 60
)

// extractOttieSessionID extracts the session UUID from a full session key.
// Returns the UUID and true if the key matches the Ottie session pattern.
func extractOttieSessionID(key string) (string, bool) {
	if strings.HasPrefix(key, ottieSessionPrefix) {
		return strings.TrimPrefix(key, ottieSessionPrefix), true
	}
	return "", false
}

func extractOttieSessionIDFromSanitizedKey(key string) (string, bool) {
	if strings.HasPrefix(key, sanitizedOttieSessionPrefix) {
		return strings.TrimPrefix(key, sanitizedOttieSessionPrefix), true
	}
	return "", false
}
...
func (h *Handler) readLegacySession(dir, sessionID string) (sessionFile, error) {
	path := filepath.Join(dir, sanitizeSessionKey(ottieSessionPrefix+sessionID)+".json")
...
func (h *Handler) readJSONLSession(dir, sessionID string) (sessionFile, error) {
	sessionKey := ottieSessionPrefix + sessionID
...
// It reads the workspace from config, falling back to ~/.ottie/workspace.
func (h *Handler) sessionsDir() (string, error) {
	...
	if workspace == "" {
		...
		workspace = filepath.Join(home, ".ottie", "workspace")
	}
...
// handleListSessions returns a list of Ottie session summaries.
//
//	GET /api/sessions
func (h *Handler) handleListSessions(w http.ResponseWriter, r *http.Request) {
	...
	for _, entry := range entries {
		...
		case strings.HasSuffix(name, ".jsonl"):
			sessionID, ok = extractOttieSessionIDFromSanitizedKey(strings.TrimSuffix(name, ".jsonl"))
			if !ok {
				continue
			}
		...
		case filepath.Ext(name) == ".json":
			...
			if _, statErr := os.Stat(filepath.Join(dir, base+".jsonl")); statErr == nil {
				if jsonlSessionID, found := extractOttieSessionIDFromSanitizedKey(base); found {
			...
			sessionID, ok = extractOttieSessionID(sess.Key)
			if !ok {
				continue
			}
...
func (h *Handler) handleDeleteSession(w http.ResponseWriter, r *http.Request) {
	...
	base := filepath.Join(dir, sanitizeSessionKey(ottieSessionPrefix+sessionID))
	jsonlPath := base + ".jsonl"
	metaPath := base + ".meta.json"
