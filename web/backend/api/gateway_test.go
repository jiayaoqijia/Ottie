...
import (
	...
	"github.com/jiayaoqijia/ottie/pkg/auth"
	"github.com/jiayaoqijia/ottie/pkg/config"
	"github.com/jiayaoqijia/ottie/web/backend/utils"
)
...
func TestFindOttieBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "ottie-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("OTTIE_BINARY", mockBinary)

	got := utils.FindOttieBinary()
	if got != mockBinary {
		t.Errorf("FindOttieBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindOttieBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When OTTIE_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("OTTIE_BINARY", "/nonexistent/ottie-binary")

	got := utils.FindOttieBinary()
	// Should not return the invalid path; falls back to "ottie" or another found path
	if got == "/nonexistent/ottie-binary" {
		t.Errorf("FindOttieBinary() returned invalid env path %q, expected fallback", got)
	}
}
