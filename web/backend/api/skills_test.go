...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("OTTIE_BUILTIN_SKILLS")
	if err := os.Setenv("OTTIE_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(OTTIE_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("OTTIE_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("OTTIE_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
