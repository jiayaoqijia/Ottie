...
const (
	autoStartEntryName = "OttieLauncher"
	launchAgentLabel   = "io.ottie.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "ottie-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=Ottie Web",
			"Comment=Start Ottie Web on login",
