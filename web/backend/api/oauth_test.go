...
func setupOAuthTestEnv(t *testing.T) (string, func()) {
	...
	oldOttieHome := os.Getenv("OTTIE_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("OTTIE_HOME", filepath.Join(tmp, ".ottie")); err != nil {
		t.Fatalf("set OTTIE_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldOttieHome == "" {
			_ = os.Unsetenv("OTTIE_HOME")
		} else {
			_ = os.Setenv("OTTIE_HOME", oldOttieHome)
		}
	}
