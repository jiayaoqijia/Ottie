...
func TestBuildWsURLUsesRequestHostWhenLauncherPublicSaved(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "http://launcher.local/api/ottie/token", nil)
	req.Host = "192.168.1.9:18800"

	if got := h.buildWsURL(req, cfg); got != "ws://192.168.1.9:18790/ottie/ws" {
		t.Fatalf("buildWsURL() = %q, want %q", got, "ws://192.168.1.9:18790/ottie/ws")
	}
}
