	...
	items = append(items,
		MenuItem{
			...
			Action: func() {
				...
				s.addModel(
					ottieconfig.ModelConfig{ModelName: newName, Model: "openai/gpt-5.4"},
				)
				s.push(
...
func (s *appState) addModel(model ottieconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []ottieconfig.ModelConfig) {
...
func isModelValid(model ottieconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *ottieconfig.ModelConfig) {
	if model == nil {
		return
	}
