...
import (
	...
	configstore "github.com/jiayaoqijia/ottie/cmd/ottie-launcher-tui/internal/config"
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open ottie agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("ottie", "gateway")
