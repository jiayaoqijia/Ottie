...
func (c *TelegramChannel) handleMessage(ctx context.Context, message *telego.Message) error {
	...
	if message.Chat.Type != "private" {
		...
		// Swarm mode: check for handoff blocks targeting this bot
		if c.config != nil && c.config.Swarm.Enabled {
			if handoffMsg, ok := extractHandoffForBot(content, c.bot.Username()); ok {
				content = handoffMsg
				isMentioned = true
			} else if user.IsBot && !isMentioned {
				// Skip messages from other bots that don't mention us
				logger.DebugCF("telegram", "Swarm: ignoring bot message (not mentioned)", map[string]any{
					"from_bot": user.Username,
				})
				return nil
			}
		}
...
// extractHandoffForBot parses JSON handoff blocks from a message and returns
// the handoff message if it targets this bot. Returns ("", false) otherwise.
func extractHandoffForBot(content, botUsername string) (string, bool) {
	if botUsername == "" {
		return "", false
	}

	// Look for ```json blocks containing handoff type
	...
	for _, match := range matches {
		...
		// Check if target matches this bot (with or without @)
		target := strings.TrimPrefix(handoff.To, "@")
		if strings.EqualFold(target, botUsername) {
			return fmt.Sprintf("[Handoff from %s]: %s", handoff.From, handoff.Message), true
		}
	}
