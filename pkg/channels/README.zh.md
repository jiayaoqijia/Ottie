# Ottie Channel System：完整开发指南

> **影响范围**: `pkg/channels/`, `pkg/bus/`, `pkg/media/`, `pkg/identity/`, `cmd/ottie/internal/gateway/`
...
import (
    "github.com/jiayaoqijia/ottie/pkg/bus"
    "github.com/jiayaoqijia/ottie/pkg/channels"     // 引用父包
    "github.com/jiayaoqijia/ottie/pkg/config"
    "github.com/jiayaoqijia/ottie/pkg/identity"      // 新增
    "github.com/jiayaoqijia/ottie/pkg/media"          // 新增（如需媒体）
)
```
...
import (
    _ "github.com/jiayaoqijia/ottie/pkg/channels/telegram"   // 触发 init() 注册
    _ "github.com/jiayaoqijia/ottie/pkg/channels/discord"
    _ "github.com/jiayaoqijia/ottie/pkg/channels/your_new_channel"  // 新增
)
```
...
```go
// 如果平台支持发送占位消息（如 "Pondering..."），并且实现了 MessageEditor，
// 则 Manager 的 preSend 会在出站时自动将占位消息编辑为最终回复。
// SendPlaceholder 内部根据 PlaceholderConfig.Enabled 决定是否发送；
// 返回 ("", nil) 表示跳过。
// 当 cfg.Text 为空时，channels.RandomThinkingPhrase() 会返回随机的动名词短语
//（如 "Synthesizing..."、"Spelunking..."、"Orchestrating..."）。
...
7. **ReasoningChannelID**：大多数 channel config 都包含 `reasoning_channel_id` 字段，用于将 LLM 的思维链（reasoning/thinking）路由到指定 channel（WhatsApp、Telegram、Feishu、Discord、QQ、DingTalk、Slack、LINE、OneBot、WeCom、WeComApp）。`BaseChannel` 通过 `WithReasoningChannelID` 选项和 `ReasoningChannelID()` 方法暴露此配置。