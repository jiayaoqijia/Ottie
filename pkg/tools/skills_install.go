...
import (
	...
	"github.com/jiayaoqijia/ottie/pkg/fileutil"
	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"registry": map[string]any{
				"type":        "string",
				"description": "Registry to install from (optional, defaults to first configured registry e.g., 'clawhub')",
			},
...
func (t *InstallSkillTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	// Resolve registry: use provided name or fall back to default.
	...
	if registryName != "" {
		...
		registry = t.registryMgr.DefaultRegistry()
		if registry == nil {
	...
	// Local content security scan on extracted files.
	scanResult := skills.ScanSkillContent(targetDir)
	if scanResult.HasFindings() {
		logger.WarnCF("tool", "Skill content scan found concerns",
			map[string]any{
				"tool":     "install_skill",
				"slug":     slug,
	...
	// Build result with moderation and scan warnings.
	var output string
	if result.IsSuspicious {
		output += fmt.Sprintf("⚠️ Warning: skill %q is flagged as suspicious by the registry.\n\n", slug)
	}
	if scanResult.HasFindings() {
		output += fmt.Sprintf(
			"⚠️ Security scan warning for skill %q:\n%s\n"+
				"Review the skill content before use. Proceed with caution.\n\n",
			slug, scanResult.Summary())
	}
