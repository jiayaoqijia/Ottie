...
func TestInstallSkillToolAlreadyExists(t *testing.T) {
	...
	// Must have a registry configured so we get past registry resolution.
	rm := skills.NewRegistryManagerFromConfig(skills.RegistryConfig{
		ClawHub: skills.ClawHubConfig{Enabled: true},
	})
...
func TestInstallSkillToolParameters(t *testing.T) {
	...
	// registry is now optional (defaults to first configured registry)
}

func TestInstallSkillToolNoRegistryConfigured(t *testing.T) {
	// No registries configured — should fail with "no registries configured".
	tool := NewInstallSkillTool(skills.NewRegistryManager(), t.TempDir())
	result := tool.Execute(context.Background(), map[string]any{
		"slug": "some-skill",
	})
	assert.True(t, result.IsError)
	assert.Contains(t, result.ForLLM, "no registries configured")
}

func TestInstallSkillToolExplicitRegistryNotFound(t *testing.T) {
	tool := NewInstallSkillTool(skills.NewRegistryManager(), t.TempDir())
	result := tool.Execute(context.Background(), map[string]any{
		"slug":     "some-skill",
		"registry": "nonexistent",
