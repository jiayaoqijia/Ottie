...
// ChannelPolicy controls which channels may invoke a tool.
type ChannelPolicy int

const (
	// ChannelPolicyAll allows any channel to call this tool (default).
	ChannelPolicyAll ChannelPolicy = iota
	// ChannelPolicyInternal restricts the tool to internal channels only (cli, system, subagent).
	ChannelPolicyInternal
)
...
// RegisterInternal registers a core tool restricted to internal channels only.
func (r *ToolRegistry) RegisterInternal(tool Tool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	name := tool.Name()
	if _, exists := r.tools[name]; exists {
	...
	r.tools[name] = &ToolEntry{
		...
		IsCore:        true,
		TTL:           0,
		ChannelPolicy: ChannelPolicyInternal,
	}
	r.version.Add(1)
	logger.DebugCF("tools", "Registered internal tool", map[string]any{"name": name})
}
...
// ApplyFilter removes tools that don't match the allow/deny lists.
// If allow is non-empty, only tools in the allow list are kept.
// Tools in the deny list are always removed.
// Only affects core tools; hidden (non-core) tools are untouched.
func (r *ToolRegistry) ApplyFilter(allow, deny []string) {
...
func (r *ToolRegistry) ExecuteWithContext(
	...
	// Enforce channel policy: block internal-only tools from external channels.
	if entry.ChannelPolicy == ChannelPolicyInternal && !constants.IsInternalChannel(channel) {
		logger.WarnCF("tool", "Tool blocked by channel policy",
			map[string]any{
				"tool":    name,
				"channel": channel,
			})
		return ErrorResult(fmt.Sprintf("tool %q is restricted to internal channels", name))
	}
	...
	// Redact sensitive data from tool output when channel is external.
	if !constants.IsInternalChannel(channel) {
		result.ForLLM = RedactSensitive(result.ForLLM)
		if result.ForUser != "" {
			result.ForUser = RedactSensitive(result.ForUser)
		}
	}
