...
var (
	defaultDenyPatterns = []*regexp.Regexp{
		...
		// Block commands that leak environment variables and credentials.
		regexp.MustCompile(`\benv\b`),
		regexp.MustCompile(`\bprintenv\b`),
		regexp.MustCompile(`\bexport\s+-p\b`),
		regexp.MustCompile(`\bset\b\s*$`),
		regexp.MustCompile(`\bcat\b.*\.env\b`),
		regexp.MustCompile(`\bcat\b.*/etc/(shadow|passwd)\b`),
		regexp.MustCompile(`\bcat\b.*\.ssh/`),
		regexp.MustCompile(`\bcat\b.*config\.json\b`),
	}
...
func (t *ExecTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	// Defense in depth: strip sensitive env vars from subprocess environment.
	cmd.Env = sanitizeExecEnv()
...
var sensitiveEnvPattern = regexp.MustCompile(
	`(?i)(API_?KEY|TOKEN|SECRET|PASSWORD|PRIVATE_?KEY|CREDENTIAL)`,
)

// sensitiveEnvPrefixPattern matches env var names with well-known secret prefixes.
var sensitiveEnvPrefixPattern = regexp.MustCompile(
	`(?i)^(AWS_|GH_|GITHUB_|OPENAI_|ANTHROPIC_|OTTIE_)`,
)

// sanitizeExecEnv returns a filtered copy of os.Environ() with sensitive
// variables removed. Applied to all exec subprocesses as defense in depth.
func sanitizeExecEnv() []string {
	env := os.Environ()
	...
	for _, kv := range env {
		...
		if sensitiveEnvPattern.MatchString(key) || sensitiveEnvPrefixPattern.MatchString(key) {
			continue
		}
