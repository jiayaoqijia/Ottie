...
func buildRequestBody(
	...
	for _, msg := range messages {
		...
		case "user":
			if msg.ToolCallID != "" {
				// Tool result — merge into the previous user message if it
				// already contains tool_result blocks (Anthropic requires all
				// tool_results for one assistant turn in a single user message).
				toolResult := map[string]any{
					"type":        "tool_result",
					"tool_use_id": msg.ToolCallID,
					"content":     msg.Content,
				}
				if merged := mergeToolResult(apiMessages, toolResult); merged {
					// already appended into previous user message
				} else {
		...
		case "assistant":
			...
			for _, tc := range msg.ToolCalls {
				// Resolve name: prefer tc.Name, fall back to Function.Name
				name := tc.Name
				if name == "" && tc.Function != nil {
					name = tc.Function.Name
				}

				// Resolve input: prefer tc.Arguments, fall back to parsing Function.Arguments
				input := tc.Arguments
				if input == nil {
					...
					// Anthropic requires input to be a valid dictionary, never null
					if input == nil {
		...
		case "tool":
			// Tool result — merge into the previous user message if possible
			toolResult := map[string]any{
				"type":        "tool_result",
				"tool_use_id": msg.ToolCallID,
				"content":     msg.Content,
...
// mergeToolResult appends a tool_result block into the last apiMessage if it is
// a user message that already contains tool_result content. This ensures all
// tool results for one assistant turn are sent in a single user message, as
// required by the Anthropic Messages API.
func mergeToolResult(apiMessages []any, toolResult map[string]any) bool {
	if len(apiMessages) == 0 {
		return false
	}
	last, ok := apiMessages[len(apiMessages)-1].(map[string]any)
	if !ok || last["role"] != "user" {
		return false
	}
	// Check if content is a slice (tool_result array), not a plain string
	switch content := last["content"].(type) {
	case []map[string]any:
		if len(content) > 0 {
			if t, ok := content[0]["type"]; ok && t == "tool_result" {
