...
func TestClassifyError_ModelNotFound(t *testing.T) {
	...
	for _, msg := range patterns {
		...
		if result.Reason != FailoverModelNotFound {
			t.Errorf("pattern %q: reason = %q, want model_not_found", msg, result.Reason)
		}
		if result.IsRetriable() {
			t.Errorf("pattern %q: model_not_found should not be retriable", msg)
		}
	}
}
...
func TestClassifyError_SessionExpired(t *testing.T) {
	...
	result := ClassifyError(err, "openai", "gpt-4")
	if result == nil {
		t.Fatal("expected non-nil for session expired")
	}
	if result.Reason != FailoverSessionExpired {
		t.Errorf("reason = %q, want session_expired", result.Reason)
	}
}

func TestClassifyError_CauseChain(t *testing.T) {
	inner := &FailoverError{
	...
	result := ClassifyError(outer, "openai", "gpt-4")
	if result == nil {
		t.Fatal("expected non-nil for wrapped FailoverError")
	}
	if result != inner {
		t.Error("expected the original inner FailoverError to be returned")
	}
	if result.Reason != FailoverRateLimit {
