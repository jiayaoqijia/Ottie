...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "ottie-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
// (~/.ottie/skills) invalidates the cached system prompt.
func TestGlobalSkillFileContentChange(t *testing.T) {
	...
	globalSkillPath := filepath.Join(tmpHome, ".ottie", "skills", "global-skill", "SKILL.md")
...
func TestBuiltinSkillFileContentChange(t *testing.T) {
	...
	t.Setenv("OTTIE_BUILTIN_SKILLS", builtinRoot)
	...
	for g := range goroutines {
		...
		go func(id int) {
			...
			for i := range iterations {
				...
				if !strings.Contains(result, "ottie") {
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "ottie-bench-*")
	defer os.RemoveAll(tmpDir)
