...
type AgentInstance struct {
	...
	// SwarmManager is non-nil for orchestrator agents with swarm capabilities.
	SwarmManager *tools.SwarmManager
}
...
func NewAgentInstance(
	...
	// Apply per-agent tool filters (allow/deny lists from config)
	if agentCfg != nil && (len(agentCfg.ToolsAllow) > 0 || len(agentCfg.ToolsDeny) > 0) {
		toolsRegistry.ApplyFilter(agentCfg.ToolsAllow, agentCfg.ToolsDeny)
	}
	...
	).WithSwarm(cfg.Swarm.Enabled, cfg.Swarm.InstanceID)
	...
	// Look up model config by model_name alias or by scanning model_list
	// for a matching Model field (handles resolved model names like "gpt-5.4").
	var thinkingLevelStr string
	var forceStream bool
	if mc, err := cfg.GetModelConfig(defaults.GetModelName()); err == nil {
		thinkingLevelStr = mc.ThinkingLevel
		forceStream = mc.ForceStream
	} else {
		// Fallback: scan model_list for a matching Model field
		for i := range cfg.ModelList {
			if cfg.ModelList[i].Model == model || strings.HasSuffix(cfg.ModelList[i].Model, "/"+model) {
				thinkingLevelStr = cfg.ModelList[i].ThinkingLevel
				forceStream = cfg.ModelList[i].ForceStream
				break
			}
		}
	}
...
func resolveAgentWorkspace(agentCfg *config.AgentConfig, defaults *config.AgentDefaults) string {
	...
	// Use the configured default workspace (respects OTTIE_HOME)
...
// Close releases resources held by the agent's session store and swarm manager.
