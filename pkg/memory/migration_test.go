...
func TestMigrateFromJSON_SkipsMetaJSONFiles(t *testing.T) {
	...
	if addErr := store.AddMessage(ctx, "agent:main:ottie:direct:ottie:test", "user", "keep me"); addErr != nil {
		t.Fatalf("AddMessage: %v", addErr)
	}
	if summaryErr := store.SetSummary(ctx, "agent:main:ottie:direct:ottie:test", "keep summary"); summaryErr != nil {
		t.Fatalf("SetSummary: %v", summaryErr)
	}

	metaPath := filepath.Join(sessionsDir, "agent_main_ottie_direct_ottie_test.meta.json")
	...
	history, err := store.GetHistory(ctx, "agent:main:ottie:direct:ottie:test")
	if err != nil {
		t.Fatalf("GetHistory: %v", err)
	}
	...
	summary, err := store.GetSummary(ctx, "agent:main:ottie:direct:ottie:test")
	if err != nil {
		t.Fatalf("GetSummary: %v", err)
	}
