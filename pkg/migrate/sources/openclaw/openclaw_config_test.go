...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.ottie/workspace" {
		t.Errorf("expected workspace '~/.ottie/workspace', got '%s'", workspace)
	}
...
func TestConvertToOttie(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	ottieCfg, warnings, err := cfg.ConvertToOttie("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}

	if ottieCfg.Agents.Defaults.ModelName != "claude-sonnet-4-20250514" {
		t.Errorf("expected model 'claude-sonnet-4-20250514', got '%s'", ottieCfg.Agents.Defaults.ModelName)
	}
	if ottieCfg.Agents.Defaults.Workspace != "~/.ottie/workspace" {
		t.Errorf("expected workspace '~/.ottie/workspace', got '%s'", ottieCfg.Agents.Defaults.Workspace)
	}

	if len(ottieCfg.Agents.List) != 2 {
		t.Errorf("expected 2 agents, got %d", len(ottieCfg.Agents.List))
	}
	if ottieCfg.Agents.List[0].ID != "main" {
		t.Errorf("expected first agent id 'main', got '%s'", ottieCfg.Agents.List[0].ID)
	}
	if ottieCfg.Agents.List[1].Skills == nil || len(ottieCfg.Agents.List[1].Skills) != 2 {
		t.Errorf("expected 2 skills for assistant agent")
	}

	if !ottieCfg.Channels.Telegram.Enabled {
		t.Error("telegram should be enabled")
	}
	if ottieCfg.Channels.Telegram.Token != "test-token" {
		t.Errorf("expected telegram token 'test-token', got '%s'", ottieCfg.Channels.Telegram.Token)
	}

	if ottieCfg.Channels.WhatsApp.BridgeURL != "http://localhost:3000" {
		t.Errorf("expected whatsapp bridge URL 'http://localhost:3000', got '%s'", ottieCfg.Channels.WhatsApp.BridgeURL)
	}

	if ottieCfg.Channels.Feishu.AppID != "app-id" {
		t.Errorf("expected feishu app ID 'app-id', got '%s'", ottieCfg.Channels.Feishu.AppID)
	}

	if len(ottieCfg.ModelList) != 1 {
		t.Errorf("expected 1 model config (no models.json provided), got %d", len(ottieCfg.ModelList))
	}
...
func TestConvertToOttieWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	ottieCfg, _, err := cfg.ConvertToOttie("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}

	if !ottieCfg.Channels.QQ.Enabled {
		t.Error("qq should be enabled")
	}
	if ottieCfg.Channels.QQ.AppID != "qq-app-id" {
		t.Errorf("expected qq app ID 'qq-app-id', got '%s'", ottieCfg.Channels.QQ.AppID)
	}

	if !ottieCfg.Channels.DingTalk.Enabled {
		t.Error("dingtalk should be enabled")
	}
	if ottieCfg.Channels.DingTalk.ClientID != "ding-app-id" {
		t.Errorf("expected dingtalk client ID 'ding-app-id', got '%s'", ottieCfg.Channels.DingTalk.ClientID)
	}

	if !ottieCfg.Channels.Slack.Enabled {
		t.Error("slack should be enabled")
	}
	if ottieCfg.Channels.Slack.BotToken != "xoxb-test" {
		t.Errorf("expected slack bot token 'xoxb-test', got '%s'", ottieCfg.Channels.Slack.BotToken)
	}
	if ottieCfg.Channels.Slack.AppToken != "xapp-test" {
		t.Errorf("expected slack app token 'xapp-test', got '%s'", ottieCfg.Channels.Slack.AppToken)
	}
}

func TestConvertToOttieWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	if !ottieCfg.Channels.Matrix.Enabled {
		t.Error("matrix should be enabled")
	}
	if ottieCfg.Channels.Matrix.Homeserver != "https://matrix.example.com" {
		t.Errorf("expected matrix homeserver, got %q", ottieCfg.Channels.Matrix.Homeserver)
	}
	if ottieCfg.Channels.Matrix.UserID != "@bot:matrix.example.com" {
		t.Errorf("expected matrix user_id, got %q", ottieCfg.Channels.Matrix.UserID)
	}
	if ottieCfg.Channels.Matrix.AccessToken != "syt_test_token" {
		t.Errorf("expected matrix access_token, got %q", ottieCfg.Channels.Matrix.AccessToken)
	}
	if len(ottieCfg.Channels.Matrix.AllowFrom) != 1 ||
		ottieCfg.Channels.Matrix.AllowFrom[0] != "@alice:matrix.example.com" {
		t.Errorf("unexpected matrix allow_from: %#v", ottieCfg.Channels.Matrix.AllowFrom)
	}
...
func TestConvertToOttieWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	stdCfg := ottieCfg.ToStandardConfig()
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.ottie/workspace" {
		t.Errorf("expected workspace '~/.ottie/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
