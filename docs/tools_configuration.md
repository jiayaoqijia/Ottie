...
Ottie's tools configuration is located in the `tools` field of `config.json`.
...
By default, Ottie blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `OTTIE_TOOLS_<SECTION>_<KEY>`:

For example:

- `OTTIE_TOOLS_WEB_BRAVE_ENABLED=true`
- `OTTIE_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `OTTIE_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `OTTIE_TOOLS_MCP_ENABLED=true`
