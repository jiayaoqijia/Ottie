...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Ottie.
...
10. [Creating a New Provider in Ottie](#creating-a-new-provider-in-ottie)
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<OttieConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- Ottie codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.ottie/auth.json`:
...
## Creating a New Provider in Ottie

Ottie providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/ottie/cmd_auth.go`:
...
ottie auth login --provider your-provider
...
ottie agent -m "Hello" --model your-model
```
...
export OTTIE_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export OTTIE_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `ottie auth login --provider antigravity`
...
- Check auth profile storage: `~/.ottie/auth.json`
- Re-run `ottie auth login --provider antigravity`
