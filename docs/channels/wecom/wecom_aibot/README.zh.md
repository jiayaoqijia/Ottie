...
4. 将 Token 和 EncodingAESKey 填入 Ottie 配置文件，启动服务后回到管理后台保存（企业微信会发送验证请求）
...
用户发消息
  │
  ▼
Ottie 立即返回 {finish: false}（Agent 开始处理）
  │
  ▼
...
若 Agent 处理时间超过约 30 秒（企业微信最大轮询窗口为 6 分钟），Ottie 会：
...
配置 `welcome_message` 后，当用户打开与 AI Bot 的聊天窗口时（`enter_chat` 事件），Ottie 会自动回复该欢迎语。留空则静默忽略。

```json
"welcome_message": "你好！我是 Ottie AI 助手，有什么可以帮你？"
```
...
- 检查 Ottie 日志是否收到了来自企业微信的 GET 请求
